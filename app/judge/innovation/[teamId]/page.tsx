...
export default async function judge({
  ...
  // convert the URL parameter into a number that will be used as the database team id
  const teamId = parseInt(params.teamId);
  const supabase = createClient();
  // query supabase with this id
  const { data: evaluationFromDB } = await supabase
    ...
    <>
      ...
      <section className="max-w-screen-sm m-8">
        ...
        <p className="mt-2">
          ...
          the Innovation Project presentation. Judges are required to tick one
          box on each separate row to indicate the level the team has achieved.
          If the team EXCEEDS, a short comment in the exceeds column is required
        </p>
      </section>
      <EvaluationGrid
        evalAxes={evalAxes}
        data={evaluationFromDB ? evaluationFromDB[0] : null}
        teamId={teamId}
      />
    </>
  );
